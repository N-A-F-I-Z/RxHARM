...
# Replace the last code cell (parallel execution)
nb["cells"][-1]["source"] = [
    ...
    "\n",
    "    # Sum child bands (age 0 + age 1 = under 5)\n",
    "    child_sum = np.zeros_like(total_pop)\n",
    "    for key in ['child_00', 'child_01']:\n",
    ...
    "    try:\n",
    "        # Try new file-path API first, fall back to array API\n",
    "        try:\n",
    ...
    "            )\n",
    "        except TypeError:\n",
    "            # Fallback for older installed version\n",
    "            with rasterio.open(viirs_path) as src:\n",
    "                v_arr = src.read(1).astype(np.float32)\n",
    "                v_prof = src.profile.copy()\n",
    "            viirs_100m, fine_profile = downscaler.downscale(\n",
    "                viirs_coarse_arr=v_arr,\n",
    "                coarse_profile=v_prof,\n",
    "                pop_fine_arr=total_pop.astype(np.float32),\n",
    "            )\n",
